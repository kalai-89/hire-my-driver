...
public class UpdateDelete extends AppCompatActivity {
    ...
    Button Update_offer, Delete_offer;
    String description, quantity, price, offer, CustomerId;
    String RandomUId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Offername = (TextView) findViewById(R.id.type_name1);
        imageButton = (ImageButton) findViewById(R.id.image_upload);
        Update_offer = (Button) findViewById(R.id.Updatetype);
        Delete_offer = (Button) findViewById(R.id.Deletetype);
        ID = getIntent().getStringExtra("updatedeleteoffer");
        ...
        dataaa.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    public void onClick(View v) {
                        ...
                        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                firebaseDatabase.getInstance().getReference("OfferDetails").child(State).child(City).child(Sub).child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(ID).removeValue();

                                AlertDialog.Builder food = new AlertDialog.Builder(UpdateDelete.this);
                                food.setMessage("Your Offer has been Deleted");
                                food.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog, int which) {

                                        startActivity(new Intent(UpdateDelete.this, customer_panel_bottom_navigation.class));
                                    }
                                });
                ...
                databaseReference = FirebaseDatabase.getInstance().getReference("OfferDetails").child(State).child(City).child(Sub).child(useridd).child(ID);
                databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        UpdateModel updateModel = dataSnapshot.getValue(UpdateModel.class);

                        desc.getEditText().setText(updateModel.getDescription());
                        qty.getEditText().setText(updateModel.getQuantity());
                        Offername.setText("Offer name: " + updateModel.getOffers());
                        offer = updateModel.getOffers();
                        pri.getEditText().setText(updateModel.getPrice());
                        Glide.with(UpdateDelete.this).load(updateModel.getImageURL()).into(imageButton);
                        dburi = updateModel.getImageURL();
        ...
        if (imageuri != null) {
            ...
            ref.putFile(imageuri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onFailure(@NonNull Exception e) {
                    ...
                    Toast.makeText(UpdateDelete.this, "Failed : " + e.getMessage(), Toast.LENGTH_LONG).show();
                }
    ...
    private void updatedesc(String uri) {
        ...
        OfferDetails info = new OfferDetails(offer, quantity, price, description, uri, ID, CustomerId);
        firebaseDatabase.getInstance().getReference("OfferDetails").child(State).child(City).child(Sub)
                .child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(ID)
                .setValue(info).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        progressDialog.dismiss();
                        Toast.makeText(UpdateDelete.this, "Offer Updated Successfully", Toast.LENGTH_SHORT).show();
                    }
                });
    }
